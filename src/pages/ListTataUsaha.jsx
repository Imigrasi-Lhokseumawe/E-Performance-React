...
const ListTataUsaha = () => {
  ...
  const handleAccept = async (e, userId) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/tata-usaha/${userId}`, jsonData, {
        headers: {
                      ...
                      {tataUsaha.length === 0 ? (
                        ...
                        tataUsaha.map((tataUsaha, index) => (
                          <tr key={tataUsaha.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>{tataUsaha.kegiatan}</td>
                            <td style={{ textAlign: 'center' }}>{tataUsaha.jumlah}</td>
                            <td style={{ textAlign: 'center' }}>{tataUsaha.output}</td>
                            <td style={{ textAlign: 'center' }}>{formatCurrency(tataUsaha.anggaran)}</td>
                            <td style={{ textAlign: 'center' }}>{formatCurrency(tataUsaha.sisaAnggaran)}</td>
                            <td style={{ textAlign: 'center' }}>{tataUsaha.periode}</td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              ...
                              {user && user.role === "admin" && !tataUsaha.isAccept && (
                                <button
                                  type="button"
                                  className="btn btn-success btn-sm"
                                  onClick={(e) => handleAccept(e, tataUsaha.uuid)}
                                >
